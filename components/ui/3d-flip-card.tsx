...
const Card = ({
  ...
  const isTopCardInStack = frontCardIndex === null && index === 0;
  const isSelectedFront = isFront === true;

  return (
    <motion.div
      className={cn(
        "absolute overflow-hidden rounded-xl shadow-lg group/card cursor-pointer",
        isFront && "z-20"
      )}
      style={{
        width,
    ...
    >
      <Image
        ...
        className="rounded-xl grayscale group-hover/card:grayscale-0 transition-all duration-300"
      />
      {(isTopCardInStack || isSelectedFront) && (
        <motion.div 
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          className="absolute bottom-4 right-4 bg-black/60 backdrop-blur-md p-2 rounded-full hidden group-hover/card:block"
        >
